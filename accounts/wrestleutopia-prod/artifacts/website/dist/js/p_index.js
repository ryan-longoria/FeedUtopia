import { a as apiFetch } from "./chunks/core.js";
...
const needsBust = (k) => /^public\/promoters\/profiles\//.test(String(k)) || /^profiles\//.test(String(k));
  ...
  containerEl.innerHTML = `
    ...
    return `
                <div class="media-card">
                  ${isYT ? `<iframe width="100%" height="220" src="${h(src).replace("watch?v=", "embed/")}" title="Video" frameborder="0" allowfullscreen></iframe>` : `<video src="${h(src)}" controls></video>`}
                </div>`;
  }).join("")}
          </div>
