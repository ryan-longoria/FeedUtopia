...
export async function getAuthState() {
  try {
    ...
    const groups = Array.isArray(payload['cognito:groups'])
      ...
      : (payload['cognito:groups'] ? String(payload['cognito:groups']).split(/[,\s]+/) : []);
    const roleClaim = (payload['custom:role'] || '').toLowerCase();
    const role = roleClaim.startsWith('promo') ? 'Promoter' :
                 roleClaim.startsWith('wrestl') ? 'Wrestler' : null;
...
export const isPromoter = (s) => s.groups?.includes('Promoters') || s.role === 'Promoter';
export const isWrestler = (s) => s.groups?.includes('Wrestlers') || s.role === 'Wrestler';
...
export async function guardPage({ requireRole, redirect = 'index.html', replace = null } = {}) {
  const s = await getAuthState();
  const ok =
    !requireRole ||
  ...
  if (replace) {
    ...
    if (target) target.innerHTML = `<p class="muted">Not authorized for this section.</p>`;
    return s;
  }
