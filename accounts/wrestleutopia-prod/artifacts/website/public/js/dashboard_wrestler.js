...
function scoreTryout(t, profile) {
  ...
  if (profile) {
    const cityMatch =
      profile.city && t.city &&
      String(profile.city).toLowerCase() === String(t.city).toLowerCase();
...
async function loadRecommendedTryouts() {
  ...
  try {
    ...
    renderEmptyTryouts(
      target,
      transient
        ? { title: "We’re having trouble loading tryouts", desc: "This might be a temporary issue. Please refresh in a moment." }
        : undefined
    );
...
async function loadMyApplications() {
  ...
  try {
    ...
    apps
      .sort((a, b) =>
        new Date(b.timestamp || b.createdAt || b.created_at || 0) -
        new Date(a.timestamp || a.createdAt || a.created_at || 0)
      )
      .slice(0, 6)
