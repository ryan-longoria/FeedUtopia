...
const PAGE_CORR_ID = (globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`).toString();

function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

async function robustFetch(path, { method = "GET", body, headers = {}, timeoutMs = 8000, retries = 2 } = {}) {
  ...
  try {
    ...
    try { console.error(JSON.stringify({ evt: "dash_api_error", corr: PAGE_CORR_ID, path, msg })); } catch { console.error("dash_api_error", PAGE_CORR_ID, path, msg); }
    throw err;
  } finally {
    clearTimeout(timer);
  }
...
function cacheKey(path, extra = "") { return `${CACHE_NS}:${path}:${extra}`; }
...
function setCachedJson(key, value) { try { sessionStorage.setItem(key, JSON.stringify({ t: Date.now(), v: value })); } catch {} }
...
function safeLink(href, text) { return el("a", { href, target: "_blank", rel: "noopener noreferrer", text }); }
function clearNode(n) { while (n.firstChild) n.removeChild(n.firstChild); }

const fmtDate = (iso) => { try { return new Date(iso).toLocaleDateString(); } catch { return iso || ""; } };
const daysUntil = (iso) => { const d = new Date(iso); const now = new Date(); if (isNaN(d)) return 9999; return Math.ceil((d - now) / (1000 * 60 * 60 * 24)); };

async function getMyWrestlerProfile() {
  try {
    try { const me = await robustFetch("/profiles/wrestlers/me"); if (me) return me; } catch {}
    try {
...
async function setRingNameFromProfile() {
  try {
    ...
    const el = document.getElementById("ring-name");
    if (el) el.textContent = String(ring);
  } catch (e) {
    console.debug("setRingNameFromProfile:", e?.message || e);
  }
}
...
async function loadRecommendedTryouts() {
  ...
  try {
    ...
    const cacheK = cacheKey("/tryouts", s?.sub || "anon");
    const cached = getCachedJson(cacheK, 90 * 1000);
    let profile, list;
    if (cached) {
...
async function loadMyApplications() {
  ...
  async function trySeq(urls) {
    for (const u of urls) {
      try { return await robustFetch(u); }
      catch (e) {
        const msg = String(e);
  ...
  try {
    ...
    let apps = cacheK ? getCachedJson(cacheK, 60 * 1000) : null;
    if (!apps) {
      const urls = [
