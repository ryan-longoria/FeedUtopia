...
def lambda_handler(event, context):
    ...
    if not bucket_name:
        raise ValueError("Missing TARGET_BUCKET environment variable.")
    ...
    if image_path and image_path.startswith("http"):
        try:
            ...
            print(f"Image download from URL failed: {e}")
            bg_local_path = None
    elif image_path:
        try:
            s3.download_file(bucket_name, image_path, bg_local_path)
        except Exception as e:
            print(f"Failed to download image from S3 key '{image_path}': {e}")
            bg_local_path = None
    ...
    try:
        ...
        print("Failed to download news clip:", e)
    ...
    if logo_local_path and os.path.exists(logo_local_path):
        ...
        side_margin = int(logo_clip.w + base_margin)
    else:
        logo_clip = None
    ...
    bottom_font_size = max(top_font_size - 10, 10)
    ...
    top_clip = (TextClip(title_top,  fontsize=top_font_size,  color="#ec008c", font=font_path,
                         method="caption", size=(available_width,None))
                .set_duration(duration_sec))
    bottom_clip = (TextClip(title_bottom, fontsize=bottom_font_size, color="#ec008c", font=font_path,
                            method="caption", size=(available_width,None))
                   .set_duration(duration_sec))
    desc_top_clip = (TextClip(subtitle_top, fontsize=subtitle_font_size, color="white", font=font_path,
                              method="caption", size=(available_subtitle_width,None))
                     .set_duration(duration_sec))
    desc_bottom_clip = (TextClip(subtitle_bottom, fontsize=subtitle_font_size, color="white", font=font_path,
                                 method="caption", size=(available_subtitle_width,None))
                        .set_duration(duration_sec))
