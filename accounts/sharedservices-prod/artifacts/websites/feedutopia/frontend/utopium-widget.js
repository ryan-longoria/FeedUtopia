  ...
  @media (max-width:420px){
    ...
    <div id="ut-window" role="dialog" aria-modal="true" aria-label="Utopium Chat">
      ...
      const IMG_STEPS = { prompt:0, askRef:1, file:2, gen:3 };   // image flow steps
      ...
      const askRefYN     = ()=>{ bot('Would you like to attach a reference image?'); quickReplies(['yes','no']); };
      const askRefFile   = ()=> chooseFile(f=>{ state.refFile=f; afterImgStep(); });
  
      /* ─────── Drag‑&‑drop helper (shared with bg picker) ─────── */
      function chooseFile(cb){
        bot('Please choose or drag the file below:');
      ...
      async function callImageGen(){
        ...
        try{
          ...
          // 3) invoke image‑gen Lambda
          const res=await fetch(`${API_ROOT}/gpt/image-gen`,{
            method:'POST',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({prompt:state.imgPrompt,refImageId:refId})
          });
          if(!res.ok) throw new Error(`backend ${res.status}`);
          const {url}=await res.json();
          // 4) show image
          bot(`<img src="${url}" style="width:100%;max-width:100%;border-radius:12px" alt="Generated image">`);
        }catch(err){
          bot(`<i>Error:</i> ${err.message}`);
        }finally{
      ...
      /* ─────── File chooser for post background ─────── */
      const askFile = ()=>{
      ...
      function acceptInput(text){
        ...
        if(state.changePending){
          ...
          if(tl==='restart'){ resetChat();state.hasGreeted=true;bot('What can I help you with?');quickReplies(['create post','create instagram post title & description','create image']);return; }
        ...
        if(state.restartPending){
          user(text);
          if(tl==='yes'){ resetChat();state.hasGreeted=true;bot('What can I help you with?');quickReplies(['create post','create instagram post title & description','create image']); }
          else{ state.restartPending=false;(state.step>0?steps[state.step-1]:bot('What can I help you with?')); if(state.step<=0) quickReplies(['create post','create instagram post title & description','create image']); }
          return;
        }
        ...
        if(state.step===-1){
          ...
          else if(['create image','make image','generate image'].includes(tl)){
            bot('Sure! Let’s create an image.');
            startImageFlow();
          }
          else{ bot('What can I help you with?');quickReplies(['create post','create instagram post title & description','create image']); }
          return;
        }
  ...
  Ready to publish?`;
      ...
      /* ───────── Publish (unchanged) ───────── */
      async function publish(){ /* your existing publish logic */ }
  
      /* ───────── Utilities ───────── */
      function resetChat(){
        $messages.innerHTML=''; state={ ...state,step:-1,data:{},file:null,awaitYes:false,changePending:false,editing:null,restartPending:false,skipHLSub:false,gptMode:false,gptStep:-1,imgMode:false,imgStep:-1,imgPrompt:'',refFile:null };
      }
